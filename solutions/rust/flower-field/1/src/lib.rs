pub fn annotate(garden: &[&str]) -> Vec<String> {
    ...
    let mut result: Vec<Vec<u8>> = garden_bytes.iter()
        .map(|row| row.to_vec())
        .collect();
...
fn count_adjacent_flowers(garden: &[&[u8]], row: usize, col: usize, rows: usize, cols: usize) -> u8 {
    let mut count = 0;
        ...
        for dc in [-1,0,1] {
            ...
            if new_row >= 0 && new_row < rows as i32 &&
               new_col >= 0 && new_col < cols as i32 {
                if garden[new_row as usize][new_col as usize] == b'*' {
                    count += 1;
                }
            }
        }
